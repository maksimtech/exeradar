"""
ExeRadar — local cache of EU law provisions.

Stored in ~/.exeradar/law_cache.json (or $EXERADAR_HOME/law_cache.json):
...
from exeradar.law_fetcher import Provision, text_sha256

Key = tuple[str, str]   # (celex, article)


def default_cache_path() -> Path:
    """Where the cache lives, from EXERADAR_HOME or the default under $HOME.
    ...
    """
    home = (os.environ.get("EXERADAR_HOME") or "").strip()
    if not home:
        return Path.home() / ".exeradar" / "law_cache.json"
