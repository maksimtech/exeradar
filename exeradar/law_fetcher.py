"""
ExeRadar — EU and Italian law text from EUR-Lex and Normattiva.

Downloads the acts ExeRadar reports cite and extracts their articles, down to
...
~/.exeradar/law_cache.json.

Shared by the Radar tools; the first version, in APKRadar, read the GDPR only.

ExeRadar added two things, both forced rather than chosen:

- EU acts are fetched from the Publications Office's Cellar service, with the
  eur-lex.europa.eu page as the fallback. The page now answers automated
  requests with HTTP 202 and an AWS WAF challenge (`x-amzn-waf-action:
  challenge`) for every act, so it is not a source any more. Cellar serves the
  same documents, in the same ELI markup, and exists for machine access.
- annexes can be addressed, as "Allegato I, Parte I" and its points. The
  Cyber Resilience Act keeps its requirements in Annex I and its articles only
  point at it, so without this ExeRadar would cite the pointer and not the rule.
"""
...
# content negotiation. Unlike the EUR-Lex web interface it is not behind a bot
# challenge, and it returns the same ELI markup the parser below expects.
CELLAR_URL = "https://publications.europa.eu/resource/celex/{celex}"
CELLAR_TYPE = "application/xhtml+xml"
# Cellar negotiates in ISO 639-2; the family speaks of acts in two letters.
...
# Regulation (EU) 2024/2847. Its cybersecurity requirements are in Annex I;
# articles 6 and 13 place the duty and point at the annex for its content.
CRA = Act("Cyber Resilience Act reg. (UE) 2024/2847", "32024R2847")
...
# An annex is addressed as "Allegato I, Parte I": the part belongs in the
# reference because Part I and Part II are both numbered from (1), so the
# number alone names two different provisions.
_ANNEX_UNIT = re.compile(
    r"^(?:Allegato|Annex)\s+([IVXL]+)(?:\s*,\s*(?:Parte|Part)\s+([IVXL]+))?$", re.I
)
# The heading that opens a part, in either language of the two the family reads.
_PART_TITLE = re.compile(r"^(?:Parte|Part)\s+([IVXL]+)\b", re.I)
# Annex points are labelled "1)" and "a)" in the Italian rendering and "(1)"
# and "(a)" in the English one. Same document, same service: the label shape
# is a property of the translation, so both are accepted.
_ANNEX_LABEL = re.compile(r"^\(?(\d+|[a-z]{1,4})\)$", re.I)


def _direct_rows(table: _Element) -> list[_Element]:
    """Rows of `table` itself, never those of a table nested in a cell."""
    rows = []
    for child in table.children:
        ...
        if child.tag == "tr":
            ...
            rows.extend(c for c in child.children if isinstance(c, _Element) and c.tag == "tr")
    return rows


def _annex_point(table: _Element) -> Optional[tuple[str, str]]:
    """(label, text) of a point laid out as a label cell and a text cell.

    A numbered point is indented one cell further than a lettered one, so the
    label is looked for rather than assumed to be first.
    """
    ...
    for index, cell in enumerate(cells[:-1]):
        match = _ANNEX_LABEL.match(_text(cell))
        if match:
            body = " ".join(_text(c) for c in cells[index + 1:])
            return match.group(1), normalize_text(f"{_text(cell)} {body}")
    return None


def _parse_annex(root: _Element, unit: str) -> dict[str, str]:
    """Text of an annex part, its numbered points and their lettered points.

    The div id is `anx_I` whatever the language, so the annex is found by id
    and only the part heading has to be read as words.
    """
    match = _ANNEX_UNIT.match(unit)
    if match is None:
        return {}
    annex_id = f"anx_{match.group(1).upper()}"
    wanted = match.group(2).upper() if match.group(2) else None

    annex = next((e for e in _elements(root) if e.attrs.get("id") == annex_id), None)
    if annex is None:
        return {}
    ...
    for child in annex.children:
        if not isinstance(child, _Element):
            continue
        if child.tag == "table":
            if wanted is not None and part != wanted:
                continue
            point = _annex_point(child)
            if point is None:
                continue
            number, text = point
            ref = f"{unit}({number})"
            texts[ref] = text
            lines.append(text)
            for nested in _elements(child):
                if nested.tag != "table" or nested is child:
                    continue
                letter = _annex_point(nested)
                if letter is not None:
                    texts[f"{ref}({letter[0]})"] = letter[1]
            continue
        title = _PART_TITLE.match(_text(child))
        if title:
            part = title.group(1).upper()
            continue
        if wanted is None or part == wanted:
            text = _text(child)
            if text:
                lines.append(text)
...
def parse_articles(html: str, articles: tuple[str, ...]) -> dict[str, str]:
    ...
    for number in articles:
        if _ANNEX_UNIT.match(number):
            parsed = _parse_annex(builder.root, number)
            if not parsed.get(number):
                raise LawFetchError(f"{number} not found in the page")
            texts.update(parsed)
            continue
...
def fetch_act_html(
    ...
    *,
    ...
    The order is not a preference. eur-lex.europa.eu now sits behind an AWS
    WAF challenge that answers every automated request with HTTP 202 and an
    empty body, whichever act is asked for, so it cannot be the first choice
    any more. It is kept as the fallback because a WAF rule can be relaxed
    again and the page is the same text.
